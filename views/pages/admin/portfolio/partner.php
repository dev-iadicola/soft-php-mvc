                        ...
                        <?php foreach ($partners as $item) : ?>
                            <div class="list-group-item d-flex justify-content-between align-items-center gap-3" data-id="<?= $item->id ?>" style="<?= $item->is_active ? '' : 'opacity: 0.5;' ?>">
                                ...
                                <div class="d-flex gap-2">
                                    <button type="button" class="btn btn-<?= $item->is_active ? 'success' : 'secondary' ?> btn-sm toggle-active-btn" onclick="toggleActive('partner', <?= $item->id ?>, this)"><?= $item->is_active ? 'Attivo' : 'Archiviato' ?></button>
