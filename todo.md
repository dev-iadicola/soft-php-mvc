si gr# TODO - Potenziale del Portfolio
...
- `Attributes::__get()` e `__set()` non sono ancora completamente allineati con il nuovo flusso a proprieta tipizzate.
...
- [ ] Ricontrollare tutti i model e allineare i tipi alle colonne attese
...
- [ ] Aggiungere stub per model, controller, middleware, migration e seeder con le convenzioni attuali
...
- [ ] Aggiungere un comando per ispezionare le rotte scoperte
- [ ] Aggiungere un comando per ispezionare colonne del model e nome tabella inferito
- [ ] Migliorare consistenza dell'output CLI e messaggi di errore
...
- [ ] Aggiungere test per caricamento rotte, ereditarieta, stack middleware e matching
- [ ] Introdurre cache delle rotte per produzione
- [ ] Migliorare il reporting degli errori quando gli attributi sono malformati
- [ ] Aggiungere supporto per nomi rotta, URL generation e ispezione delle rotte da CLI
- [ ] Standardizzare le firme dei metodi dei controller e l'iniezione di `Request`

#### Fase 4.b: Refactor Routing in stile Spatie

- [ ] Introdurre attributi HTTP separati: `#[Get]`, `#[Post]`, `#[Put]`, `#[Patch]`, `#[Delete]`
- [ ] Mantenere `#[RouteAttr]` come layer di compatibilita temporaneo
- [ ] Aggiungere attributi di gruppo come `#[Prefix]`, `#[Middleware]` e `#[NamePrefix]`
- [ ] Permettere al `RouteLoader` di ignorare i metodi pubblici senza attributi di routing
- [ ] Rimuovere la rigidita per cui ogni metodo pubblico del controller deve essere una rotta
...
- [ ] Aggiungere URL generation basata su route name e parametri
- [ ] Aggiungere un comando CLI `route:list`
- [ ] Aggiungere test di compatibilita tra nuovi attributi HTTP e `RouteAttr`
- [ ] Aggiungere test su prefix multipli, merge middleware e inheritance dei controller
...
- [ ] Creare helper di validazione orientati alla request o classi stile form-request
- [ ] Aggiungere accessor tipizzati in `Request`
- [ ] Supportare payload validati normalizzati per booleani, interi, array e file
- [ ] Migliorare l'estendibilita del validator per regole custom
- [ ] Standardizzare il trasporto degli errori di validazione verso sessione e viste
- [ ] Aggiungere test di validazione su edge case e flussi nullable
...
- [ ] attributi HTTP dedicati
- [ ] compatibilita con `RouteAttr`
- [ ] `route:list`
- [ ] URL generation
