...
class ClassControllers implements IteratorAggregate {
    ...
    public function reflectionController(string $className): ReflectionClass {
        $controllerName = $this->stacks[$className];
        return new ReflectionClass($controllerName);
    }

    public function reflectionAllControllers(): array {
        $controllerRelfected = [];
     ...
     * @return void
     */
    public function setStack(string $className, Stack $stack): void {
